...
def build_prompts(
    *,
    ...
    system_prompt = (
        ...
        "and data_gaps. stock_watch is a fixed monitoring pool, not a "
        "recommendation list. Every sector_watch and stock_watch entry MUST "
        "carry evidence_ids that reference event IDs from the stage1 digest. "
        "Never introduce Taiwan tickers outside the fixed pool. Output is a "
    ...
    user_prompt = (
        ...
        "- stock_watch is fixed to these tickers only: 2330 台積電, 2317 鴻海, "
        "2454 聯發科, 2308 台達電, 2881 富邦金, 2882 國泰金, "
        "2485 兆赫, 3535 晶彩科, 3715 定穎投控, 2351 順德.\n"
        "- Use 4-digit ticker codes only. Do not output .TW/.TWO suffixes and do not add substitutes.\n"
        "- For pre_tw_open and delivery-eligible us_close, map evidence onto the fixed pool. "
        "If a fixed-pool name lacks evidence, list the gap instead of replacing it with another ticker.\n"
        ...
        "- Keep sector_watch 0-6 items; stock_watch 0-10 fixed-pool items only.\n\n"
