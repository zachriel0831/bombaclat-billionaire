...
- Scope: Collect recent finance, society, politics, celebrity, and Free Palestine context from existing storage, let a Codex automation summarize the last four hours, store versioned digests in Redis with a 4h10m TTL, keep the latest display value until replacement, and expose it through `news-platform-api`.
