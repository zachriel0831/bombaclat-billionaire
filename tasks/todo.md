...
## Current Task
- Task: Add a four-hour Codex-generated cross-section news digest cached in Redis.
- Requested by: user
- Start date: 2026-06-28
- Scope: Collect recent finance, society, politics, celebrity, and Free Palestine context from existing storage, let a Codex automation summarize the last four hours, store the latest digest in Redis with a 4h10m TTL, and expose it through `news-platform-api`.

## Plan
- [x] Read repo instructions, project index, workflow docs, existing digest/API/Redis paths, and source-storage boundaries.
- [x] Add `NEWS-9` spec and update requirement/project documentation.
- [x] Add context collection and Redis store scripts under `data-collecting`.
- [x] Add API endpoint that reads the latest four-hour digest from Redis without failing hard when Redis is unavailable.
- [x] Create the Codex four-hour automation task.
- [x] Run focused verification, commit, and push related repos.
...
- Redis keys use `news:digest:four-hour:latest` for API reads and a versioned key plus `news:digest:four-hour:current-key` for controlled replacement. The TTL is 15,000 seconds.
