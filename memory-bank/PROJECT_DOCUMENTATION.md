...
- Uses official TWSE openapi dataset `t187ap04_L` (`銝??砍瘥?之閮`)
- Tracks allowlisted listed-company codes from `TWSE_MOPS_TRACKED_CODES`
- Market analysis stock-recommendation generation is retired; daily analysis must not output `stock_watch`, buy/watchlist candidates, or entry/stop/target lists.
...
- Reads Taiwan society/politics RSS, sitemap, ETtoday category-list, PTS category-page, Storm public channel pages, and low-frequency public HTML list sources defined in `news_platform.registry`
- Default active society/politics loop source set is LTN, ETtoday, CNA, PTS, EBC, Newtalk, and Storm Media. Storm uses `/channel/9` for society/domestic coverage and `/channel/7` for politics because the legacy RSS API can return empty channels. `NEWSPF_DISABLED_SOURCE_IDS` defaults to `tvbs,ctee,udn,setn` so the live 15-minute loop does not poll broken or low-frequency endpoints.
- TVBS, UDN, and SETN are registered as `html_list` sources and should be run through the low-frequency path: one-shot `scripts/run_news_platform_low_frequency_sources.ps1` / CLI `--source-ids tvbs,udn,setn`, or scheduled `scripts/register_news_platform_low_frequency_sources_task.ps1 -StartNow`. The scheduled path is a visible fixed window, not a popup-and-exit command window. TVBS uses public category JSON-LD; UDN uses society `6639` and politics-adjacent `6638`; SETN uses public `ViewAll.aspx` category pages and filters by the visible category label.
...
- Data flow:
  ...
  4. deterministic classifier writes up to three specific topic hits into `topics_json`; `TopicSpec.categories` scopes category-specific rules such as politics L2 topics to `category=politics`; no-hit rows become category-specific general topics (`general_social_news` / 銝?祉冗???or `general_politics_news` / 銝?祆瘝餅?? with `source=rule_fallback`, `topic_classified_by=rule`
      - `low_birthrate` includes common policy variants such as `撠?憟喳?`, `0??甇脣?摰嗡?韏琿?`, and `??TISA`
      ...
      - Related-link sections in summaries (`撱嗡撓?梯?`, `?賊??啗?`, `?游??啗?`) are removed during RSS summary cleaning and ignored before deterministic matching to avoid persisting/searching/classifying unrelated articles
...
- Current public-record sources:
  - Legislative Yuan legal proposals (`ly_bills`): `https://www.ly.gov.tw/WebAPI/LegislativeBill.aspx`, stored as `source_id=ly`, `record_type=legislative_bill`, `category=politics`; upstream ROC dates are normalized to Asia/Taipei timestamps; collection uses at least a 365-day lookback because the official API can be quiet for more than 14 days
  ...
  - CWA typhoon and earthquake reports (`cwa_typhoon_report`, `cwa_earthquake_report`, included in `cwa_weather`): Central Weather Administration OpenData API, stored as `source_id=cwa`, `category=society`, tagged `weather`; configured through local `.env` key `CWA_AUTHORIZATION`, with optional `CWA_TYPHOON_DATASET_ID`, legacy single `CWA_EARTHQUAKE_DATASET_ID`, and comma-separated `CWA_EARTHQUAKE_DATASET_IDS`; earthquake collection defaults to both significant felt earthquakes (`E-A0015-001`) and small-area felt earthquakes (`E-A0016-001`); typhoon parsing handles both named typhoons and active tropical depressions such as `CwaTdNo` records with analysis/forecast data, so pre-naming TD systems can still appear on the weather page
...
- `scripts/register_news_platform_low_frequency_sources_task.ps1` registers `NewsCollector-NewsPlatformLowFrequencySources` as an interactive-logon fixed window that loops every 60 minutes by default and writes `runtime/status/news-platform-low-frequency-window-status.json`.
- `scripts/register_international_homepage_headlines_task.ps1` registers `NewsCollector-InternationalHomepageHeadlines` as an interactive-logon fixed window that loops every 60 minutes by default and writes `runtime/status/international-homepage-headlines-window-status.json`.
- `scripts/register_news_source_accuracy_audit_task.ps1` registers `NewsCollector-NewsSourceAccuracyAudit` every 2 hours by default. `scripts/register_market_analysis_tasks.ps1` also registers it so a full local schedule rebuild keeps the audit. Repeating business collectors should be fixed windows or background services; never introduce a visible popup-and-exit scheduled command window for platform business logic.
- `scripts/restart_live_services.ps1` starts `event_relay.main`, `news_collector.relay_bridge`, and `news_platform.main --loop` in three visible `-NoExit` PowerShell windows. Before each restart it closes existing Python workers plus stale worker wrapper windows, so repeated restarts do not leave empty consoles.
- `scripts/run_live_service_monitor_window.ps1` keeps `scripts/monitor_live_services.ps1` in one visible local window; `scripts/register_live_service_monitor_task.ps1` registers `NewsCollector-LiveServiceMonitor` at interactive logon instead of a repeating short-lived popup task. Each fixed-window cycle also runs `scripts/run_service_auto_repair_watch.ps1 -LaunchAgent`, which monitors the frontend, API, LINE relay, stock monitor, Redis, event relay, frontend ngrok binding, Codex Observer, enabled `NewsCollector-*` task results, data-source health, and the latest source-accuracy report. Warn/stale/missing/error probes create a deduped incident in `runtime/service-auto-repair/` and start a background `codex exec` repair agent.
...
- Event-driven SEC/TWSE-MOPS row age is treated as informational because quiet disclosure periods do not imply collector failure. Truth Social account row age is also informational once historical rows exist because duplicate/no-new-post polls do not refresh relay event `created_at`. U.S. index/snapshot and stored-analysis probes are market-calendar and due-time aware, so weekend/holiday or not-yet-due rows do not create repair incidents.
...
- Current behavior:
  ...
  - Python no longer creates or migrates trade-signal/recommendation tables
