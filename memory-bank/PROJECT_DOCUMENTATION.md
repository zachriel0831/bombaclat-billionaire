...
- `scripts/register_news_source_accuracy_audit_task.ps1` registers `NewsCollector-NewsSourceAccuracyAudit` every 2 hours by default. `scripts/register_market_analysis_tasks.ps1` also registers it so a full local schedule rebuild keeps the audit. Short-lived scheduled collectors and audits run with hidden PowerShell windows; only fixed-window monitors such as CWA and live-service monitoring should stay visible.
