...
- Daily formatting uses date-only `raw_json.display_title` and the refreshed author-style flow `今日一句話` -> `三個檢查點` -> `市場押注與預期差` -> `國際消息到台股的傳導` -> `看錯的條件` -> `備註`; `三個檢查點` must contain exactly three bullets connecting source fact -> market mechanism -> why it matters now. Do not write a dedicated `台股配置` section.
